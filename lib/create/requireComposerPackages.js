...
var description = exports.description = 'install additional PHP composer packages';

var requirements = exports.requirements = [allRequirements.composer];

var prompts = exports.prompts = [allPrompts.composerRepos, allPrompts.composerPackages, allPrompts.acfProKey];

function run(answers) {
  var composerJson = require(_path2.default.join(process.cwd(), 'composer.json'));
  composerJson.repositories = (0, _unionWith2.default)(composerJson.repositories, [repos.flyntCore, repos.acfFieldGroupComposer], _isEqual2.default);
  ...
  if (answers.composerRepos) {
    var composerRepos = answers.composerRepos.map(function (repo) {
      return {
        type: 'composer',
        url: repo
      };
