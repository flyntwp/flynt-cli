...
const description = exports.description = 'setup new theme from flynt-theme repo';

const runMessage = exports.runMessage = 'Setting up new theme...';

const requirements = exports.requirements = [allRequirements.git, allRequirements.yarn];

const prompts = exports.prompts = [allPrompts.themeName];

function run(answers) {
  const themePath = `web/app/themes/${answers.themeName}`;
  let cmds = [`git clone --depth=1 "git@github.com:bleech/wp-starter-theme.git" ${themePath}`, `ln -nfs ${themePath} theme`, `cd ${themePath}`, 'rm -rf .git'];
