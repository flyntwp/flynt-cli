...
const runMessage = exports.runMessage = 'Cloning database...';

const requirements = exports.requirements = [allRequirements.mysql, allRequirements.mysqldump, allRequirements.php, allRequirements.ssh, allRequirements.scp];
...
function getReplaceInFilePath() {
  ...
  const modulePath = _path2.default.dirname(moduleMainPath);
  const packageJsonPath = _path2.default.join(modulePath, 'package.json');
  const packageJson = require(packageJsonPath);
  return _path2.default.join(modulePath, packageJson.bin);
}