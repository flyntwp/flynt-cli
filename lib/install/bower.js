...
const description = exports.description = 'install theme dependencies with bower';

const runMessage = exports.runMessage = 'Installing bower dependencies...';
...
function run(answers) {
  ...
  if (_fs2.default.existsSync(_path2.default.join(process.cwd(), themePath, 'bower.json'))) {
    let cmds = [`cd ${themePath}`, 'bower install'];
    return (0, _executeCommand2.default)(cmds);
  } else {
    return _bluebird2.default.reject(new _Errors.SubcommandSkip('bower', `Skipping bower install. No bower.json found in ${themePath}`));
  }
}