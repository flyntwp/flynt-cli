...
function handleCommand(commandObject, fromEnv, toEnv, subCommand) {
  return function (argv) {
    ...
    switchToRoot(validCommandsValues[0].notInRootFolder || argv.force).then(function () {
      ...
      _bluebird2.default.resolve(validCommandsValues).tap(checkRequirements).then(promptMissingConfig(answersFromConfig, fromEnv, toEnv)).tap(runCommands(commandsToRun), function (err) {
