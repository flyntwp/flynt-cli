...
function handleCommand(commandObject, fromEnv, toEnv, subCommand) {
  return function (argv) {
    ...
    const validCommandsValues = _lodash2.default.values(validCommands);
    const commandsToRun = _lodash2.default.map(validCommandsValues, 'run');
    ...
    switchToRoot(validCommandsValues[0].notInRootFolder || argv.force).then(function () {
      ...
      _bluebird2.default.resolve(validCommandsValues).tap(checkRequirements).then(promptMissingConfig(answersFromConfig, fromEnv, toEnv)).tap(runCommands(commandsToRun, argv), err => console.log(err));
    });
  };
}
...
function filterValidCommands(commands, subCommands) {
  if (!subCommands) {
    ...
    return _lodash2.default.pickBy(commands, (cmd, name) => _lodash2.default.includes(subCommands, name));
  }
}
