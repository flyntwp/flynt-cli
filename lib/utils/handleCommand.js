...
function handleCommand(commandObject, fromEnv, toEnv, subCommand) {
  return function (argv) {
    ...
    _bluebird2.default.resolve(commandsToRun).tap(checkRequirements).then(promptMissingConfig(answersFromConfig)).tap((0, _config.saveConfig)(argv, config, fromEnv, toEnv)).tap(runCommands(commandsToRun), function (err) {
