...
function handleCommand(commandObject, fromEnv, toEnv, subCommand) {
  return function (argv) {
    ...
    var validCommands = filterValidCommands(commandObject.commands, subCommand);
    var validCommandsValues = _lodash2.default.values(validCommands);
    var commandsToRun = _lodash2.default.map(validCommandsValues, 'run');

    switchToRoot(validCommandsValues[0].notInRootFolder || argv.force).then(function () {
      var config = (0, _config.getConfig)(argv);
      var answersFromConfig = (0, _config.mapConfigToAnswers)(config, fromEnv, toEnv);

      var whenToSaveConfigIndex = getWhenToSaveConfigIndex(validCommandsValues);
      commandsToRun.splice(whenToSaveConfigIndex, 0, (0, _config.saveConfig)(argv, config, fromEnv, toEnv));

      _bluebird2.default.resolve(validCommandsValues).tap(checkRequirements).then(promptMissingConfig(answersFromConfig, fromEnv, toEnv)).tap(runCommands(commandsToRun, argv), function (err) {
...
function promptMissingConfig(answersFromConfig, fromEnv, toEnv) {
  return function (commands) {
    ...
    prompts = _lodash2.default.union.apply(_lodash2.default, _toConsumableArray(prompts)).map(function (prompt) {
