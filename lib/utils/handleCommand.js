...
function handleCommand(commandObject, fromEnv, toEnv, subCommand) {
  return function (argv) {
    ...
    switchToRoot(_lodash2.default.values(validCommandsValues)[0].notInRootFolder || argv.force).then(function () {
      ...
      _bluebird2.default.resolve(validCommandsValues).tap(checkRequirements).then(promptMissingConfig(answersFromConfig, fromEnv, toEnv)).tap(runCommands(commandsToRun, argv, commandObject), err => console.log(err));
    });
  };
}
  ...
  return function (answers) {
    ...
    commands.forEach(function (command, commandIndex) {
      allRuns = allRuns.then(function () {
        const spinner = (0, _ora2.default)(`Command ${commandObject.name}:${command.name}`).start();
        if ((0, _log.is)('DEBUG')) {
          spinner.stopAndPersist({ symbol: '▶' });
        }
        return _bluebird2.default.resolve(command.run(answers, argv)).then(() => {
          spinner.succeed();
        }, e => {
